...
public class SerializationUtil {
    ...
    // the map contains an entry for each user-defined class and its classification
    // depends
    ...
    static {
        ...
        type_category_map.put("com.per.msa.LoginController", "USER_REMOTE_TYPE");
        type_category_map.put("com.per.msa.LoginService", "USER_REMOTE_TYPE");
        type_category_map.put("com.per.msa.LoginDAO", "USER_REMOTE_TYPE");
    ...
    public static String encode(Object object, String type) {
        ...
        try {
            if ("bytearray".equals(type)) { // byte[] 추가
                final byte[] byteArray = (byte[]) object;
                encodedStr = Base64.getEncoder().encodeToString(byteArray) + "::" + type;
    ...
    /**
     * Encodes the given object (of non-primitive type) to a string based on dynamic
     * type of the object,
     ...
     * - other cases (scalar non-user type, array of primitive type): serialized
     * object
     * 
    ...
    public static String encodeWithDynamicTypeCheck(Object object) throws CardinalException {
        ...
        if (isUserType(objType.getName())) {
            ...
            if (!(object instanceof KluInterface)) {
                throw new CardinalException(
                        "User type does not implement KluInterface: " + object.getClass().getName());
            }
        ...
        if (object instanceof byte[]) {
            return encode(object, "bytearray");
        }

        // if array of non-primitive types, convert to array of strings, then encode the
        // array
        if (objType.isArray()) {
        ...
        // for other cases (scalar non-user type, array of primitive type), serialize
        // the object
        return encode(object, null);
    }

    /**
     * Decodes the given string, based on dynamic type, into an object, handling
     * various cases:
     * - klu string constant: decoded to null value
     * - user-type: decoded to physical object (from cluster object manager) or
     * proxy object
     ...
     * - other cases (scalar non-user type, array of primitive type): deserialized
     * object
     * 
    ...
    public static Object decodeWithDynamicTypeCheck(String encodedStr) throws CardinalException {
        ...
        if ("bytearray".equals(type)) {
            String byteString = encodedStr.split("::")[0];
        ...
        if (!(object instanceof byte[]) && objType.isArray()) {
        ...
        // for other cases (scalar non-user type, array of primitive type), return
        // deserialized object
        return object;
    }

    /**
     * Encodes the given list of objects to a list of strings. It iterates over each
     * object in list and
     * encodes it to a string by recursively calling encodeWithDynamicTypeCheck(),
     * handling the cases where the object is null or an instance of user-local
     * type,
     * user-remote type, or non-user type and generating the suitable string in each
     * case.
     * 
    ...
    /**
     * Decodes the given list of srings to a list of objects. It iterates over each
     * string in list,
     * which could represent a reference ID or a serialized non-user object, and
     * converts it into
     * a physical or proxy object.
     * 
     * @param strList List of strings representing reference IDs or serialized
     *                objects
     * @return Collection of objects
     * @throws CardinalException if exception thrown while creating collection
    ...
    /***
     * Encodes the given map of objects to a map of strings. It iterates over each
     * (key, value) pain in map and
     * encodes the key/value to a string, handling the cases where the object is
     * null or an instance of user-local type,
     * user-remote type, or non-user type and generating the suitable string in each
     * case.
     * 
    ...
    /**
     * Decodes the given map of srings to a map of objects. It iterates over each
     * (key, value) in list,
     * where the key/value could represent a reference ID or a serialized non-user
     * object, and converts it into
     * a physical or proxy object.
     * 
     * @param strMap Map of strings representing reference IDs or serialized objects
     * @return Map of object pairs
     * @throws CardinalException if exception thrown while creating map object via
     *                           reflection
     */
